...
#define TLC5947_BITS_PER_CHIP (TLC5947_BITS_PER_CH * TLC5947_CH_PER_CHIP) // 288
...
static void pack_frame_msbfirst(const tlc5947_t *dev) {
    ...
    size_t bit_idx = 0; // index bitu v dev->tx (MSB-first na drátu v rámci bajtu)
    for (int chip = dev->chips - 1; chip >= 0; --chip) {
        ...
        for (int ch = TLC5947_CH_PER_CHIP; ch > 0; --ch) {
            const uint16_t v = dev->gs[base + (ch-1)] & 0x0FFFu; // 12bit hodnota kanálu

            // MSB -> LSB (bity 11..0)
            for (int b = 11; b >= 0; --b) {
                if (v & (1u << b)) {
                    const size_t byte = bit_idx >> 3;
                    const int    bit  = 7 - (bit_idx & 7); // SPI posílá MSB bitu bajtu jako první
...
esp_err_t tlc5947_init(tlc5947_t *dev, const tlc5947_config_t *cfg) {
    ...
    spi_bus_config_t bus = {
        ...
        .flags = SPICOMMON_BUSFLAG_IOMUX_PINS // use if on default pins; harmless otherwise
    };
    // Bus init (ok i když už existuje – vrátí ESP_ERR_INVALID_STATE)
    esp_err_t err = spi_bus_initialize(cfg->host, &bus, cfg->dma_chan ? cfg->dma_chan : SPI_DMA_CH_AUTO);
...
void tlc5947_deinit(tlc5947_t *dev) {
    ...
    if (dev->tx) { free(dev->tx); dev->tx = NULL; }
    if (dev->gs) { free(dev->gs); dev->gs = NULL; }
}
...
uint16_t tlc5947_u8_to_u12_gamma(uint8_t x) {
    ...
    // (x/255)^2.2 * 4095 ≈ exp(2.2*ln(x/255))*4095 ; tady použitá LUT-less approximace
    // pro embedded jednodušeji: (x*x*x) s přepočtem
